(() => {
  ...
  kintone.events.on(["app.record.index.show"], async function (event) {
      ...
      if (userOrgAry.includes(config.orgCode.HRManagement)) {
        ...
        const result = await Swal.fire({
          ...
          didOpen: () => {
            ...
            sweerAlertBk.style.background = `#000000`; //背景
          },
          //パスワード認証
          preConfirm: async () => {
            //入力社員番号
            ...
            for (const record of records) {
              authObj[record.社員番号.value] = {
                pass: record.パスワード.value,
    ...
    if (Number(event.viewId) === Number(config.viewId.top)) {
      ...
      //それ以外 → プロフィール一覧に遷移
      location.href = `${config.url.base}/${kintone.app.getId()}/?view=${
      ...
      loggingInMenu.onclick = () => {
        ...
        if (loggingInMenuList) {
          ...
          if (loginObj.id === config.others.HRId) {
            ...
            document
              .getElementsByClassName("logging-in-menu-list")[0]
              .insertBefore(addItem, document.getElementById("logout"));
            ...
            document.getElementById("my-profile").onclick = async () => {
              ...
              //画面遷移
              location.href = `${config.url.base}/${kintone.app.getId()}/show#record=${
    ...
    if (event.viewId === config.viewId.profileList && loginObj.id !== config.others.HRId) {
      ...
      for (const row of Array.from(employeeElm)) {
        if (row.children[0].children[0].innerText === loginObj.id) {
          ...
          //自レコードを表すクラス付与
          targetRow.classList.add("myself-profile");
          break;
        }
      }
      //一致行が存在するかの確認
      ...
      for (const row of Array.from(statusElm)) {
        if (row.children[0].children[0].innerText !== "承認済") {
          //自分のレコードは除外
          if (!row.parentElement.classList.contains("myself-profile")) {
  ...
  async function logout() {
    ...
    if (movePortal.isConfirmed) {
      ...
      //"ログイン画面"に遷移（初期表示一覧に移動）
      sessionStorage.removeItem("login"); //ログイン情報の削除
      location.href = `${config.url.base}/${kintone.app.getId()}/?view=${config.viewId.top}`;
      return;
